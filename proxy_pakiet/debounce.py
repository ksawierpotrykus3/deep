...
def deduplicate_tool_results_in_prompt(messages: list[dict], state: dict) -> list[dict]:
    ...
    for i, msg in enumerate(result):
        ...
        # Try to identify which tool call this result belongs to
        # by checking content patterns for Read/Glob/Grep signatures
...
def extract_original_goal(messages: list[dict]) -> str:
    """Extract the PRIMARY user task from the first user message."""
    for msg in messages:
