...
* **Rozwiązanie:**
  1. Wyciek usunięty z 3 ścieżek: `generate_official`, non-stream oraz web `generate`. Znaczniki `<!-- PROXY_SID:... -->` nie są już wstrzykiwane do żadnej odpowiedzi.
  2. `conv_key` ustabilizowano na bazie `legid.user` (stabilna tożsamość z JWT) + hash pierwszego `<user_input>` — konwersacja nie potrzebuje już znacznika wodnego do powiązania tur.
