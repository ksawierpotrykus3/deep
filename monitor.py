...
SLOW_AFTER = 120.0       # bez tokenu > tyle = slow (timeout)
DEAD_AFTER = 300.0       # bez serca > tyle = dead (martwy)
KILL_SLOW_AFTER = 360.0  # slow (bez tokenu) dłużej niż tyle = auto-kill + restart
...
def get_snapshot() -> list[dict]:
    ...
    now = time.time()
    with _lock:
        # Pruning: usuń sesje zakończone dawniej niż 5 minut temu, zapobiegając wyciekowi pamięci
        to_prune = [
            sid for sid, s in _sessions.items()
            if s.get("finished") and (now - s.get("finished_at", s.get("started_at", 0))) > 300.0
        ]
        for sid in to_prune:
            _sessions.pop(sid, None)
            _stop_events.pop(sid, None)
            _hard_stops.pop(sid, None)

        out = []
        for sid, s in _sessions.items():
            c = dict(s)
            c["state"] = _derive_state(s)
            c["age"] = round(now - s["started_at"], 1)
            out.append(c)
...
def _writer_loop() -> None:
    ...
    while True:
        ...
        has_active = False
        with _lock:
            has_active = any(not s.get("finished") for s in _sessions.values())
            curr_count = len(_sessions)

        # Zapisuj gdy są aktywne sesje, zmieniła się liczba sesji lub raz na 60s w bezczynności
        if has_active or curr_count != last_count or idle_ticks >= 12:
            _save_snapshot()
